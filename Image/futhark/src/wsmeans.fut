...
def MIN_MOVE_RATIO: f32 = 0.005f32
...
def initialize_assignments [n] [k] (points: [n][3]f32) (clusters: [k][3]f32): [n]i64 =
  map (\p -> 
    let dists = map (\c -> dist_sq p c) clusters
    in (reduce_comm (\(bi, bd) (i, d) -> if d < bd then (i, d) else (bi, bd))
                    (0, f32.inf)
                    (zip (iota k) dists)).0
  ) points
...
def quantize_wsmeans(input_pixels: [][3]u8)(starting_clusters: []i32): (i64, []i32, []i32) =
    ...
    let cluster_indices_init = initialize_assignments points cluster_init
    let total_weight = reduce_comm (+) 0 (map i64.i32 counts)
    let (final_indices, final_clusters, _, _) =
      ...
      while iteration < MAX_ITERATIONS && !converged do
        ...
        let best_indices = map (\ds ->
            (reduce_comm (\(bi, bd) (idx, d) -> if d < bd then (idx, d) else (bi, bd))
                   (0i64, f32.inf)
                   (zip (iota cluster_count) ds)).0
          ) distances
        let move_mask = map2 (!=) best_indices cluster_indices
        let moved_weight = reduce_comm (+) 0 (map3 (\m c cnt -> if m then i64.i32 cnt else 0) move_mask cluster_indices counts)
        let move_ratio = f32.i64 moved_weight / f32.i64 total_weight
        let converged' = move_ratio < MIN_MOVE_RATIO && iteration > 0
