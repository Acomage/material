...
extern lean_object *lean_create_image_obj(uint32_t w, uint32_t h, uint32_t c,
                                          lean_object *data);
...
LEAN_EXPORT lean_obj_res lean_load_image(b_lean_obj_arg path_obj) {
  ...
  lean_object *image_obj = lean_create_image_obj(
      (uint32_t)width, (uint32_t)height, (uint32_t)channels, byte_array);
