      ...
      while iteration < MAX_ITERATIONS && !converged do
        ...
        in if converged' && iteration > 0 then
          (cluster_indices', cluster, iteration + 1, true)
            ...
            let cluster_weights =
              reduce_by_index (replicate cluster_count 0u32) (+) 0 cluster_indices' counts
            let sum_l = reduce_by_index (replicate cluster_count 0f32) (+) 0 cluster_indices' (map2 (\p c -> p[0] * f32.u32 c) points counts)
            let sum_a = reduce_by_index (replicate cluster_count 0f32) (+) 0 cluster_indices' (map2 (\p c -> p[1] * f32.u32 c) points counts)
            let sum_b = reduce_by_index (replicate cluster_count 0f32) (+) 0 cluster_indices' (map2 (\p c -> p[2] * f32.u32 c) points counts)
            let safe_weights = map (\w -> if w == 0 then 1u32 else w) cluster_weights
            let cluster' =
            map4 (\sl sa sb w -> if w == 0 then zero_cluster else [sl / f32.u32 w, sa / f32.u32 w, sb / f32.u32 w])
