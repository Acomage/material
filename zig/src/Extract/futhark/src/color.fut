...
def clamp (x: f32) : u32 = if x < 0f32 then 0u32 else if x > 255f32 then 255u32 else u32.f32 x
...
def lab_to_rgb (lab: [3]f32) : [3]u32 =
...
def int_to_rgb (c: u32) : [3]f32 =
  [ f32.u32 ((c >> 16) & 0xFF),
    f32.u32 ((c >> 8) & 0xFF),
    f32.u32 (c & 0xFF) ]

def rgb_to_int (rgb: [3]u32) : u32 =
  0xff000000u32 | ((rgb[0] & 0xFF) << 16) | ((rgb[1] & 0xFF) << 8) | (rgb[2] & 0xFF)

def int_to_lab : u32 -> [3]f32 =
  int_to_rgb >-> rgb_to_lab

def lab_to_int : [3]f32 -> u32 =
