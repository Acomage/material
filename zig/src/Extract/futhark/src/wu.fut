...
def norm ((r, g, b, w): v4) : [3]f32 =
  if w == 0 then [0, 0, 0]
  else
    let winv = 1 / f32.i64 w
    in map (\c -> f32.i64 c * winv) [r, g, b]

def quantize_wu [n] (max_colors: i64) (pixels: [n][3]u8) : [][3]f32 =
  ...
  let colors =
    map (\idx -> norm (vol (final_cubes[idx]) moments4)) (iota used)
