...
def ranked_suggestions (colors:[]i32)(populations:[]i32): ([]i32, []f32) =
  let colors_cam = map int_to_cam colors
  let hues = map (.hue) colors_cam
  let huesi64 = map i64.f32 hues
  let polulation_sum = f32.i32 (i32.sum populations)
  ...
  let (_, sorted_colors, sorted_hues) =
    unzip3 (radix_sort_float_by_key (\s -> -s.0) f32.num_bits f32.get_bit (zip3 scored_cams colors hues))
